...
接下来让 DockCat 能够加载你的小猫资源包。请阅读自定义指导 [CatCustomization.md](CatCustomization.md)，或者让你的本地 AI 助手帮你完成加载设置。

DockCat 如果允许加载不完整的自定义资源包来方便你预览实际效果，缺失或加载失败的资源类型会自动用默认小猫填充，避免屏幕上出现一只隐形猫猫。
...
- 使用统计，如陪伴时长、完成喝水/走动提醒次数、小猫出门得到的纪念品。
- 你自定义的小猫资源包。
